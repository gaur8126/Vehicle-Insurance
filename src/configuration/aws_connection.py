...
class S3Client:
    ...
    def __init__(self, region_name = REGION_NAME):
        ...
        """
        This class gets aws credentials from env_variable and creates an connection with s3
        ...
        if S3Client.s3_resource == None or S3Client.s3_client == None:
            ...
            if __access_key_id is None:
                raise Exception(f"Environment variable: {os.getenv("AWS_ACCESS_KEY_ID")} is not set")
            if __secret_access_key is None:
                raise Exception(f"Environment variable: {os.getenv("AWS_SECRET_ACCESS_KEY")} is not set")
